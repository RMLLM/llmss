# Copy and modify https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py
""" PyTorch LLaMA model."""
import math
...
LlamaRMSNorm.load_no_bias = load_layer_norm_no_bias
...
class LlamaMLP(nn.Module):
    def __init__(self, prefix, config: LlamaConfig, weigths):
        super().__init__()
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, prefix, config: LlamaConfig, weights):
        super().__init__()
        self.config = config
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, prefix, config: LlamaConfig, weights):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(prefix=f"{prefix}.self_attn", config=config, weights=weights)
        self.mlp = LlamaMLP(prefix=f"{prefix}.mlp", config=config, weigths=weights)
        self.input_layernorm = LlamaRMSNorm.load_no_bias(
            prefix=f"{prefix}.input_layernorm", weights=weights, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = LlamaRMSNorm.load_no_bias(
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, weights):
        super().__init__(config)
        self.embed_tokens = TensorParallelEmbedding(prefix="model.embed_tokens", weights=weights)
        self.layers = nn.ModuleList(
            [
                LlamaDecoderLayer(prefix=f"model.layers.{_}", config=config, weights=weights)
                for _ in range(config.num_hidden_layers)
            ]
        )
        # self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.norm = LlamaRMSNorm.load_no_bias(prefix=f"model.norm", weights=weights, eps=config.rms_norm_eps)
